...
def simulation(
    ...
    while True:
        ...
        for car in vehicles:
            ...
            if (
                car.status
                not in ["available", "waiting to charge", "waiting at start", "waiting to select"]
                and car.point == 0
            ):
                ...
                for car in vehicles:
                    ...
                    if car.status == "to shelf" or car.status == "return shelf":
                        ...
                        if len(car.charge_mission) > 0 and car.charge_mission[-1]["loc"] == (
                            car.x,
                            car.y,
                        ):
                            staying_cars.append(car.id)
            ...
            AGVInfo.append(
                {
                    ...
                    "color": car.color,
                    "status": car.status,
                    "battery": "%.1f%%" % (car.battery / FULL_CHARGE * 100),
                    "target": target,
                }
            )
            ...
            if car.status == "available" and order_left_to_assign == 0:
                ...
                if car.point == len(car.path) - 1:
                    if car.status == "to shelf":
                        print(
                            "car %d reached shelf %d at position (%d,%d) for the first time"
                            % (
                                car.id,
                                car.orders[-1]["shelf_id"],
                                SHELF_COORD[car.orders[-1]["shelf_id"]][0],
                                SHELF_COORD[car.orders[-1]["shelf_id"]][1],
                            )
                        )
                        available_tables = []
                        for table in table_list:
                            if order_list[car.orders[-1]["order_id"]].table == None:
                                ...
                                if (
                                    order_list[car.orders[-1]["order_id"]].table
                                    == table["table_id"]
                                    and not table["occupied"]
                                ):
                                    available_tables.append(table)
                        if available_tables:
                            target = available_tables[0]
                            for table in available_tables:
                                if abs(car.x - table["loc"][0]) + abs(
                                    car.y - table["loc"][1]
                                ) < abs(car.x - target["loc"][0]) + abs(car.y - target["loc"][1]):
                                    target = table
                            ...
                            print(
                                ...
                                % (
                                    car.id,
                                    car.orders[-1]["table_id"],
                                    car.orders[-1]["table_position"][0],
                                    car.orders[-1]["table_position"][1],
                                )
                            )
                            car.point = 0
                            ...
                            print(
                                "car %d waiting to select at position (%d, %d)"
                                % (car.id, car.x, car.y)
                            )
                        continue
                    if car.status == "to select":
                        print(
                            "car %d reached table %d at position (%d,%d)"
                            % (
                                car.id,
                                car.orders[-1]["table_id"],
                                car.orders[-1]["table_position"][0],
                        ...
                        print(
                            "car %d start selecting at position (%d, %d)" % (car.id, car.x, car.y)
                        )
                        continue

                    if car.status == "return shelf":
                        print(
                            "car %d reached shelf %d at position (%d,%d) for the second time"
                            % (
                                car.id,
                                car.orders[-1]["shelf_id"],
                                SHELF_COORD[car.orders[-1]["shelf_id"]][0],
                        ...
                        if car.check_battery():
                            ...
                            if available_stations:
                                all_shelf_status[car.orders[-1]["shelf_id"]] = (
                                    1  # 更新货架在位情况
                                )
                                ...
                                for station in available_stations:
                                    if abs(car.x - station["loc"][0]) + abs(
                                        car.y - station["loc"][1]
                                    ) < abs(car.x - target["loc"][0]) + abs(
                                        car.y - target["loc"][1]
                                    ):
                                        target = station
                                car.charge_mission.append(
                                    {"id": target["id"], "loc": target["loc"]}
                                )
                    ...
                    if car.status == "to charge":
                        car.status = "charging"
                        print(
                            "car %d start charging at position (%d, %d)" % (car.id, car.x, car.y)
                        )
                        continue
                    if car.status == "selecting":
                        ...
                        if car.selecting_process >= car.orders[-1]["tsort"]:
                            print(
                                "car %d finished selecting at position (%d, %d)"
                                % (car.id, car.x, car.y)
                            )
                            car.status = "return shelf"
                            print(
                                "car %d returning shelf %d at position (%d,%d)"
                                % (
                                    car.id,
                                    car.orders[-1]["shelf_id"],
                                    SHELF_COORD[car.orders[-1]["shelf_id"]][0],
                            ...
                            order_list[car.orders[-1]["order_id"]].shelf_status[
                                car.orders[-1]["sub_order_id"]
                            ] = "done"
                    ...
                    if car.status == "charging":
                        ...
                        if car.battery == FULL_CHARGE:
                            print(
                                "car %d finished charging at position (%d, %d)"
                                % (car.id, car.x, car.y)
                            )
                            car.status = "available"
                    ...
                    if car.status == "waiting to select":
                        ...
                        if available_tables:
                            ...
                            car.status = "to select"
                            print(
                                "car %d to select at table %d at (%d, %d)"
                                % (car.id, target["table_id"], target["loc"][0], target["loc"][1])
                            )
                            car.point = 0
                        else:
                            print(
                    ...
                    if car.status == "waiting to charge":
                        print(
                            "car %d waiting to charge at position (%d, %d)"
                            % (car.id, car.x, car.y)
                        )
                        available_stations = []
                        for station in charging_stations:
