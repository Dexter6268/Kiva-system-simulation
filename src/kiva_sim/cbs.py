...
def detect_collision(path1, path2):
    ...
    for t in range(1, n):
        ...
        if (path1[t][0], path1[t][1]) == (path2[t - 1][0], path2[t - 1][1]) and (
            path1[t - 1][0],
            path1[t - 1][1],
        ) == (path2[t][0], path2[t][1]):
            collision = {
                "loc": [(path1[t - 1][0], path1[t - 1][1]), (path1[t][0], path1[t][1])],
                "timestep": t,
            }
            return collision
    return None
...
def cbs(maps, starts, ends, directions, constraints, maxIteration):
    ...
    # 根节点
    root = {"cost": 0, "constraints": constraint_table, "paths": [], "collisions": []}
...
def cbs_prioritized(maps, starts, ends, directions, constraints, maxIteration):
    ...
    for loc, id_list in indices.items():
        if len(id_list) > 1:
            min_distance_id = 0
            distance = abs(starts[id_list[0]][0] - ends[id_list[0]][0]) + abs(
                starts[id_list[0]][1] - ends[id_list[0]][1]
            )
